...
**Variance** is a number that tells us how spread out the values in a data set are from the mean (average). 
It shows whether the numbers are close to the average or far away from it.
...
When we want to find how each data point in a given population varies or is spread out, then we use the population variance. 
It is used to give the squared distance of each data point from the population mean.

**Sample Variance**

If the population data is very large, it becomes difficult to calculate the population variance of the data set. 
In that case, we take a sample of data from the given dataset and find the variance of that dataset, which is called sample variance.
